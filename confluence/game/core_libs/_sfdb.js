			...
			if(sfdb[i-1] !== undefined){
				for(var k = 0; k < sfdb[i-1].length; k += 1){
					if(getRegisteredDuration(sfdb[i-1][k]) !== 0 ){
						//ONLY clone if the duration isn't 0.  ** GS: changed this comment from is to isn't **
 ...
 */
	...
	var getIsCharacterEliminated = function(characterName){
		console.log("eliminated char array:");
		console.log(eliminatedCharacters);
		if(eliminatedCharacters.indexOf(characterName) === -1){
	...
	/* ********************************************************** */
	/*
		START: These method added for Monte Carlo Tree Simulation
	 */
	var cloneSFDB = function(){

		return util.clone(sfdb);
	};

	// replace SFDB with new SFDB and update currentTimeStep
	var setSFDB = function(newSFDB){
		sfdb = newSFDB;
		currentTimeStep = sfdb.length - 1;
	};


	/*
	 	END: These method added for Monte Carlo Tree Simulation
	 */
	/* ********************************************************** */
